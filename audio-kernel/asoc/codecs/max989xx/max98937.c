...
//Receiver DC resistance is 13 +/-15% (10.00 --> 14.95)
//1<<27 is 134217728, for example: 5.4 / 3.33 * 134217728 = 217650370
#define EAR_RDC_MIN  (403056240)  // 10.00 / 3.33 * (1<<27)
...
static int max989xx_calib_save (uint32_t calib_value, int ch)
{
	...
	if (!IS_ERR(pfile)) {
		...
		size = vfs_write(pfile, (char *)&calib_value, sizeof(uint32_t), &pos);
		if (size < 0) {
			pr_err("%s: %s write failed, ret = %zu! \n", __func__, filename, size);
			ret = -1;
		}
		filp_close(pfile, NULL);
	} else {
...
static inline bool rdc_check_valid(uint32_t rdc, int ch)
{
	...
	pr_info("%s: ch:%d rdc=%u invalid, [%u, %u] \n", __func__, ch, rdc, rdc_min, rdc_max);
	return false;
}
	...
	if (ret == 0) {
		...
		if (max989xx_calib_save(impedance_l, MAX98927L)){
			...
			*payload = SPK_MUTE_VALUE;
			...
			pr_err("%s: calibrate failed to save the file [impedance_l]=%d \n", __func__, impedance_l);
			ret = -EIO;
			goto exit;
		}
		...
		if (max98927->mono_stereo == 3) {
			...
			if (max989xx_calib_save(impedance_r, MAX98927R)){
				...
				*(payload+1) = SPK_MUTE_VALUE;
				...
				pr_err("%s: calibrate failed to save the file [impedance_r]=%d \n", __func__, impedance_r);
				ret = -EIO;
				goto exit;
			}
		}
...
/*zhao.Pan@PSW.MM.AudioDriver.SmartPA, 2019/07/31,
 * add for applying calibration range and result to APP */
static ssize_t max989xx_dbgfs_range_read(struct file *file,
					     char __user *user_buf, size_t count,
					     loff_t *ppos)
{
	struct i2c_client *i2c = file->private_data;
...
/*zhao.Pan@PSW.MM.AudioDriver.SmartPA, 2019/07/31,
 * add for applying calibration range and result to APP */
static const struct file_operations max989xx_dbgfs_range_fops = {
	.open = simple_open,
...
/*zhao.Pan@PSW.MM.AudioDriver.SmartPA.1963539, 2019/04/18,
 * update or write register retry 5 times if return error */
			    ...
			    unsigned int reg, unsigned int val)
{
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++){
		if(i2c_states & (1 << i)){
			max98927_regmap_write(max98927->regmap[i], reg, val);
		}
	}
}
			       ...
			       unsigned int reg, unsigned int mask, unsigned int val)
{
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++){
		if(i2c_states & (1 << i)){
			max98927_regmap_update_bits(max98927->regmap[i], reg, mask, val);
		}
	}
}
		...
		if(i2c_states & (1 << i)){
			...
			switch (fmt & SND_SOC_DAIFMT_MASTER_MASK) {
				case SND_SOC_DAIFMT_CBS_CFS:
					max98927_regmap_update_bits(max98927->regmap[i], pcm_master_mode,
			...
			switch (fmt & SND_SOC_DAIFMT_INV_MASK) {
				case SND_SOC_DAIFMT_NB_NF:
					max98927_regmap_update_bits(max98927->regmap[i], pcm_mode_config,
  	...
  	for(j = 0; j < MAX_CHANNEL_NUM; j++){
		if(i2c_states & (1 << j)){
			...
			if (max98927->master) {
				...
				max98927_regmap_update_bits(max98927->regmap[j], pcm_master_mode,
			...
			max98927_regmap_update_bits(max98927->regmap[j], pcm_clock_setup,
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++){
		if(i2c_states & (1 << i)){
			...
			switch (snd_pcm_format_width(params_format(params))) {
					case 16:
						max98927_regmap_update_bits(max98927->regmap[i],
								pcm_mode_config,
				...
				max98927_regmap_update_bits(max98927->regmap[i], pcm_sample_rate_setup_1,
						MAX98927_PCM_Sample_rate_setup_1_DIG_IF_SR_Mask, sampling_rate);
				max98927_regmap_update_bits(max98927->regmap[i], pcm_sample_rate_setup_2,
...
#ifdef VENDOR_EDIT
//Le.Li@PSW.MM.AudioDriver.SmartPA, 2019/09/27, Add for debug i2c -71 err issue
static void max98927_self_recovery(struct max989xx_priv* max98927, int ch)
{
	unsigned int pcm_mode, master_mode, clock_setup, sample_rate1, sample_rate2, digital_gain, spk_gain;
	int i, idx;
	...
	for (i = 0; i < ARRAY_SIZE(max98937_reg_channel_map[idx]); i++)
		regmap_write(max98927->regmap[ch],
	...
	regmap_write(max98927->regmap[ch], MAX98937_PCM_Mode_Config, pcm_mode);
	regmap_write(max98927->regmap[ch], MAX98937_PCM_Master_Mode, master_mode);
	regmap_write(max98927->regmap[ch], MAX98937_PCM_Clock_setup, clock_setup);
	regmap_write(max98927->regmap[ch], MAX98937_PCM_Sample_rate_setup_1, sample_rate1);
	regmap_write(max98927->regmap[ch], MAX98937_PCM_Sample_rate_setup_2, sample_rate2);
	regmap_write(max98927->regmap[ch], MAX98927_AMP_volume_control, digital_gain);
	regmap_write(max98927->regmap[ch], MAX98927_Speaker_Gain, spk_gain);

	regmap_update_bits(max98927->regmap[ch], MAX98937_AMP_enables, 1, 1);
	regmap_update_bits(max98927->regmap[ch], MAX98937_Global_Enable, 1, 1);

	regmap_write(max98927->regmap[ch], MAX98937_Measurement_enables, 0x3);

	pr_info("%s: after reset!\n", __func__);
}

static struct max989xx_priv* p_max98927 = NULL;
//this function not used in this file, reset use reset_pm8008_max98927 function
void max98927_LR_reset(void)
{
	if (NULL == p_max98927) {
		pr_err("%s: error, p_max98927 is NULL, can't reset!\n", __func__);
	} else {
		max98927_self_recovery(p_max98927, 0);
		max98927_self_recovery(p_max98927, 1);
			...
			/* if (max98927->mono_stereo != 0x3){ */
				for (i = 0; i < MAX_CHANNEL_NUM; i++) {
					if(i2c_states & (0x1 << i)) {
						...
						max98927_regmap_update_bits(max98927->regmap[i], global_enable, 1, 0);
					...
					if(i2c_states & (0x1 << i)) {
						...
						if (max98927->safe_gain) {
							if (!max98927->dsm_enable){   // feedback is disable so dsm algorithm is incompleted , so reduce gain to safe value to protect speaker by power ouput control
								max98927_regmap_update_bits(max98927->regmap[i], MAX98927_AMP_volume_control,
										MAX98927_AMP_volume_control_AMP_VOL_Mask, max98927->digital_safe_gain);
								max98927_regmap_update_bits(max98927->regmap[i], spk_gain,
										MAX98927_Speaker_Gain_SPK_PCM_GAIN_Mask, max98927->spk_safe_gain);
							}
						}
						//regmap_update_bits(max98927->regmap[i], amp_enable, 1, 1);
						max98927_regmap_update_bits(max98927->regmap[i], global_enable, 1, 1);
					}
				}
			/* } */
		}
		...
		max98927->dsm_enable = !mute;
 		if (mute) {
   			for (i = 0; i < MAX_CHANNEL_NUM; i++) {
				if(i2c_states & (0x1 << i)) {
					if (max98927->bIsMax98937[i])
						max98927_regmap_write(max98927->regmap[i], MAX98937_Measurement_enables, 0x0);
					else
						max98927_regmap_write(max98927->regmap[i], MAX98927_Measurement_enables, 0x0);
				}
			}
			...
			for (i = 0; i < MAX_CHANNEL_NUM; i++) {
				if(i2c_states & (0x1 << i)) {
					...
					//Le.Li@PSW.MM.AudioDriver.SmartPA, 2019/09/27, Add for debug i2c -71 err issue
					if (max98927->bIsMax98937[i])
						max98927_regmap_write(max98927->regmap[i], MAX98937_Measurement_enables, 0x3);
					else
						max98927_regmap_write(max98927->regmap[i], MAX98927_Measurement_enables, 0x3);
					#else /* VENDOR_EDIT */
					if (max98927->bIsMax98937[i])
						ret = max98927_regmap_write(max98927->regmap[i], MAX98937_Measurement_enables, 0x3);
					else
						ret = max98927_regmap_write(max98927->regmap[i], MAX98927_Measurement_enables, 0x3);
					...
					for (retry = 0 ; retry < 3 && ret == -71; retry++) {
						...
						pr_info("%s: after reset_pm8008_max98927, ret = %d!\n", __func__, ret);
					}
					//if (retry == 3 && ret == -71) {
						//panic("%s: i2c err -71, retry 3 times can't return to normal!\n", __func__);
					//}
					#endif /* VENDOR_EDIT */
				}
			}
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	if (sel < ((1 << MAX98927_Speaker_Gain_Width) - 1)) {
		for(i = 0; i < MAX_CHANNEL_NUM; i++){
			if(i2c_states & (1 << i)){
				if (max98927->bIsMax98937[i])
					max98927_regmap_update_bits(max98927->regmap[i], MAX98937_Speaker_Gain,
							MAX98937_Speaker_Gain_SPK_PCM_GAIN_Mask, sel);
				else
					max98927_regmap_update_bits(max98927->regmap[i], MAX98927_Speaker_Gain,
	...
	if (sel < ((1 << MAX98927_Speaker_Gain_Width) - 1)) {
		if(i2c_states & MAX98927_CH0){
		    if (!max98927->bIsMax98937[MAX98927L]){
				max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98927_Speaker_Gain,
					MAX98927_Speaker_Gain_SPK_PCM_GAIN_Mask, sel);
			}else{
				max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98937_Speaker_Gain,
					MAX98937_Speaker_Gain_SPK_PCM_GAIN_Mask, sel);
			}
	...
	if (sel < ((1 << MAX98927_Speaker_Gain_Width) - 1)) {
		if(i2c_states & MAX98927_CH1){
		    if (!max98927->bIsMax98937[MAX98927R]){
				max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_Speaker_Gain,
					MAX98927_Speaker_Gain_SPK_PCM_GAIN_Mask, sel);
			}else{
				max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98937_Speaker_Gain,
						MAX98937_Speaker_Gain_SPK_PCM_GAIN_Mask, sel);
			}
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (sel <= ((1 << MAX98927_AMP_VOL_WIDTH) - 1)) {
		if(i2c_states & MAX98927_CH0){
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98927_AMP_volume_control,
					MAX98927_AMP_volume_control_AMP_VOL_Mask, sel);
		}
				     ...
				     struct snd_ctl_elem_value *ucontrol)
{
	...
	if (sel <= ((1 << MAX98927_AMP_VOL_WIDTH) - 1)) {
		if(i2c_states & MAX98927_CH1){
			max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_AMP_volume_control,
					MAX98927_AMP_volume_control_AMP_VOL_Mask, sel);
		}
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++){
		if(i2c_states & (1 << i)){
			if (max98927->bIsMax98937[i])
				max98927_regmap_update_bits(max98927->regmap[i], MAX98937_Boost_Control_0,
						MAX98937_Boost_Control_0_BST_VOUT_Mask, sel);
			else
				max98927_regmap_update_bits(max98927->regmap[i], MAX98927_Boost_Control_0,
					  ...
					  struct snd_ctl_elem_value *ucontrol)
{
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++){
		if(i2c_states & (1 << i)){
			if (max98927->bIsMax98937[i])
				max98927_regmap_update_bits(max98927->regmap[i], MAX98937_Boost_Control_1,
						MAX98937_Boost_Control_1_BST_ILIM_Mask, (sel << MAX98927_BST_ILIM_SHIFT) + 2);
			else
				max98927_regmap_update_bits(max98927->regmap[i], MAX98927_Boost_Control_1,
				...
				struct snd_ctl_elem_value *ucontrol)
{
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++){
		if(i2c_states & (1 << i)){
			if (max98927->bIsMax98937[i])
				max98927_regmap_update_bits(max98927->regmap[i], MAX98937_Speaker_source_select,
						MAX98937_Speaker_source_select_SPK_SOURCE_Mask, sel);
			else
				max98927_regmap_update_bits(max98927->regmap[i], MAX98927_Speaker_source_select,
				 ...
				 struct snd_ctl_elem_value *ucontrol)
{
	...
	for(i = 0; i < MAX_CHANNEL_NUM; i++){
		if(i2c_states & (1 << i)){
			if (max98927->bIsMax98937[i])
				max98927_regmap_update_bits(max98927->regmap[i], MAX98937_PCM_to_speaker_monomix_A,
						MAX98937_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask, sel);
			else
				max98927_regmap_update_bits(max98927->regmap[i], MAX98927_PCM_to_speaker_monomix_A,
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if(i2c_states & MAX98927_CH0){
	    if (!max98927->bIsMax98937[MAX98927L]){
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98927_PCM_to_speaker_monomix_A,
				MAX98927_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask, sel);
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98927_PCM_Rx_Enables_A,
				0xf, sel+1);
		}else{
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98937_PCM_to_speaker_monomix_A,
				MAX98937_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask, sel);
			max98927_regmap_update_bits(max98927->regmap[MAX98927L], MAX98937_PCM_Rx_Enables_A,
				0xf, sel+1);
		}
		pr_info("%s: register 0x%02X, value 0x%02X\n",
				   ...
				   struct snd_ctl_elem_value *ucontrol)
{
	...
	if(i2c_states & MAX98927_CH1){
	    if (!max98927->bIsMax98937[MAX98927R]){
			max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_PCM_to_speaker_monomix_A,
				MAX98927_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask, sel);
			max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_PCM_Rx_Enables_A, 0xf, sel+1);
			pr_info("%s: register 0x%02X, value 0x%02X\n",
				__func__, MAX98927_PCM_to_speaker_monomix_A, sel);
		} else{
			max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98937_PCM_to_speaker_monomix_A,
				MAX98937_PCM_to_speaker_monomix_A_DMONOMIX_CH0_SOURCE_Mask, sel);
			max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98937_PCM_Rx_Enables_A, 0xf, sel+1);
			pr_info("%s: register 0x%02X, value 0x%02X\n",
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if(i2c_states & MAX98927_CH0){
	    if (!max98927->bIsMax98937[MAX98927L]){
			max98927_regmap_write(max98927->regmap[MAX98927L], MAX98927_Measurement_enables, sel);
			pr_info("%s: register 0x%02X, value 0x%02X\n",
				__func__, MAX98927_Measurement_enables, sel);
		}else{
			max98927_regmap_write(max98927->regmap[MAX98927L], MAX98937_Measurement_enables, sel);
			pr_info("%s: register 0x%02X, value 0x%02X\n",
				      ...
				      struct snd_ctl_elem_value *ucontrol)
{
	...
	if(i2c_states & MAX98927_CH1){
	    if (!max98927->bIsMax98937[MAX98927R]){
			max98927_regmap_write(max98927->regmap[MAX98927R], MAX98927_Measurement_enables, sel);
			pr_info("%s: register 0x%02X, value 0x%02X\n",
				__func__, MAX98927_Measurement_enables, sel);
		}else{
			max98927_regmap_write(max98927->regmap[MAX98927R], MAX98937_Measurement_enables, sel);
			pr_info("%s: register 0x%02X, value 0x%02X\n",
				__func__, MAX98937_Measurement_enables, sel);
		}
	} else {
					     ...
					     struct snd_ctl_elem_value *ucontrol)
{
	...
	if(i2c_states & MAX98927_CH1){
	    if (!max98927->bIsMax98937[MAX98927R]){
			if (disable) {
				max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_Global_Enable,
					1, 1);
			} else {
				max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98927_Global_Enable,
					1, 0);
			...
			if (disable) {
				max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98937_Global_Enable,
					1, 1);
			} else {
				max98927_regmap_update_bits(max98927->regmap[MAX98927R], MAX98937_Global_Enable,
					1, 0);
    ...
    if (!max98927->bIsMax98937[MAX98927L]){
		if (mix) {
			...
			for (i = 0; i<ARRAY_SIZE(max98927_reg_channel_map[0]); i++)
				max98927_regmap_write(max98927->regmap[MAX98927L], max98927_reg_channel_map[0][i][0],
			...
			for (i = 0; i<ARRAY_SIZE(max98927_reg_channel_map[MAX98927L+1]); i++)
				max98927_regmap_write(max98927->regmap[MAX98927L], max98927_reg_channel_map[MAX98927L+1][i][0],
		...
		if (mix) {
			...
			for (i = 0; i<ARRAY_SIZE(max98937_reg_channel_map[0]); i++)
				max98927_regmap_write(max98927->regmap[MAX98927L], max98937_reg_channel_map[0][i][0],
					max98937_reg_channel_map[0][i][1]);
			max98927->rcv_mix_mode = true;
		} else {
			pr_info("%s: Exit Mix Mode\n", __func__);
			for (i = 0; i<ARRAY_SIZE(max98937_reg_channel_map[MAX98927L+1]); i++)
				max98927_regmap_write(max98927->regmap[MAX98927L], max98937_reg_channel_map[MAX98927L+1][i][0],
			      ...
			      const struct i2c_device_id *id)
{
		...
		switch (check_max98927_presence(max98927->regmap[id->driver_data])) {
			case MAX98927_ID:
				...
				for (i = 0; i < ARRAY_SIZE(max98927_reg_channel_map[idx]); i++)
					max98927_regmap_write(max98927->regmap[id->driver_data],
	...
	#ifdef VENDOR_EDIT
	/*Zhao.Pan@PSW.MM.AudioDriver.SmartPA, 2019/10/02,
	 * Add for max989xx reset when i2c -71 error. */
	p_max98927 = max98927;
	pr_info("set p_max98927.\n");
	#endif //VENDOR_EDIT
