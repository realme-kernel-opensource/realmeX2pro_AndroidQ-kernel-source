...
static struct qpnp_pon *gPon = NULL;
#endif/*VENDOR_EDIT*/
...
//yangmingjin@BSP.POWER.Basic 2019/05/24 add for RM_TAG_POWER_RAMDUMP
#ifdef VENDOR_EDIT
static ssize_t ramdump_by_key_show(struct kobject *kobj, struct kobj_attribute *attr, char *buf)
{
	struct qpnp_pon *pon = gPon;
	int rc = -1;
	u32 s1_timer = -1, s2_timer = -1, s2_type = -1, en_val = -1;

	if(NULL == pon){
		return snprintf(buf, 64, "pon is null\n");
	}

	rc = regmap_read(pon->regmap, QPNP_PON_KPDPWR_S1_TIMER(pon), &s1_timer);
	if (rc) {
		dev_err(pon->dev, "Unable to read PON KPDPWR_S1_TIMER\n");
		return rc;
	}

	rc = regmap_read(pon->regmap, QPNP_PON_KPDPWR_S2_TIMER(pon), &s2_timer);
	if (rc) {
		dev_err(pon->dev, "Unable to read PON KPDPWR_S2_TIMER\n");
		return rc;
	}

	rc = regmap_read(pon->regmap, QPNP_PON_KPDPWR_S2_CNTL(pon), &s2_type);
	if (rc) {
		dev_err(pon->dev, "Unable to read PON KPDPWR_S2_CNTL\n");
		return rc;
	}

	rc = regmap_read(pon->regmap, QPNP_PON_KPDPWR_S2_CNTL2(pon), &en_val);
	if (rc) {
		dev_err(pon->dev, "Unable to read PON KPDPWR_S2_CNTL2\n");
		return rc;
	}

	return snprintf(buf, 64, "s1_timer: %d s2_timer: %d s2_type: %d en_val: %d\n", s1_timer,
...
static ssize_t ramdump_by_key_store(struct kobject *kobj,
				  ...
				  const char *buf, size_t size)
{
	...
	if(NULL == pon){
		pr_err("ramdump_by_key_store pon is null\n");
		return size;
	}
	...
	rc = kstrtou32(buf, 10, &value);
	pr_info("ramdump_by_key_store value=%d\n", value);
	...
	if(!!value){
		...
		if(NULL != cfg){
			if (cfg->key_code) {
				...
				pr_info("ramdump_by_key_store disable_irq for state_irq\n");
				if (cfg->pon_type == PON_KPDPWR && cfg->support_reset){
					disable_irq(cfg->bark_irq);
					pr_info("ramdump_by_key_store disable_irq for bark_irq\n");
				}
			}
			cfg->s1_timer = 1000;
			cfg->s2_timer = 1000;
			...
			pr_err("ramdump_by_key_store cfg is null\n");
	}
	return size;
}
static struct kobj_attribute ramdump_by_key_attr =
    __ATTR(ramdump_by_key, 0644, ramdump_by_key_show, ramdump_by_key_store);
#endif/*VENDOR_EDIT*/
...
//yangmingjin@BSP.POWER.Basic 2019/05/24 add for RM_TAG_POWER_RAMDUMP
#ifdef VENDOR_EDIT
    struct qpnp_pon_config *cfg_for_resin = NULL;
#endif/*VENDOR_EDIT*/
...
//yangmingjin@BSP.POWER.Basic 2019/05/24 add for RM_TAG_POWER_RAMDUMP
#ifdef VENDOR_EDIT
	cfg_for_resin = qpnp_get_cfg(pon, PON_KPDPWR);
	if(cfg_for_resin != NULL){
		gPon = pon;
		rc = sysfs_create_file(power_kobj, &ramdump_by_key_attr.attr);
		if (rc) {
			dev_err(dev, "ramdump_by_key sys file creation failed rc: %d\n", rc);
		}
	}
#endif/*VENDOR_EDIT*/
