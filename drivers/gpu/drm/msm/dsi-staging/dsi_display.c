...
#ifdef VENDOR_EDIT
/* Zeke.Shi@RM.MM.Display.LCD.Stability,2019/9/26
 * Set the osc freq after panel on for 19688 project.
*/
...
*/
	if (clk_rate <= 800000000) {
		pr_err("%s: bitrate should be greater than 400m\n", __func__);
		return -EINVAL;
	}
#endif /* VENDOR_EDIT */
...
#ifdef VENDOR_EDIT
/* LiPing-M@PSW.MM.Display.LCD.Stability,2018/08/16
 * Add for panel OSC clk feature
*/
DEFINE_MUTEX(dynamic_osc_clock_lock);
int dynamic_osc_clock = 101600;
int tmp_dynamic_osc_clk = 101600;
static ssize_t sysfs_dynamic_osc_clk_read(struct device *dev,
...
static ssize_t sysfs_dynamic_osc_clk_write(struct device *dev,
    struct device_attribute *attr, const char *buf, size_t count)
{
    ...
    rc = kstrtoint(buf, DSI_CLOCK_BITRATE_RADIX, &osc_clk);
    if (rc) {
    ...
    if (display->panel->panel_mode != DSI_OP_CMD_MODE) {
     ...
     tmp_dynamic_osc_clk = osc_clk;
     mutex_unlock(&display->display_lock);

     return count;
}
static DEVICE_ATTR(dynamic_osc_clock, 0644,
                sysfs_dynamic_osc_clk_read,
                sysfs_dynamic_osc_clk_write);
static struct attribute *dynamic_osc_clock_fs_attrs[] = {
        &dev_attr_dynamic_osc_clock.attr,
        NULL,
    };
static struct attribute_group dynamic_osc_clock_fs_attrs_group = {
        .attrs = dynamic_osc_clock_fs_attrs,
    };
#endif /* VENDOR_EDIT */
...
*/
	if (display->panel->panel_mode == DSI_OP_CMD_MODE)
		rc = sysfs_create_group(&dev->kobj,
			&dynamic_osc_clock_fs_attrs_group);
#endif
 ...
 * Set the osc freq after panel on for 19688 project.
*/
        if (is_project(OPPO_19781) && (get_Operator_Version() == 5)) {
             if (display->config.panel_mode == DSI_OP_CMD_MODE) {
                  dsi_display_clk_ctrl(display->dsi_clk_handle,
				DSI_CORE_CLK, DSI_CLK_ON);
             }

             pr_info("%s: tmp_dynamic_osc_clk =%d,project=%d,operator=%d\n",__func__,
                          tmp_dynamic_osc_clk,is_project(OPPO_19781),get_Operator_Version());
             if (tmp_dynamic_osc_clk == 103600){
                 rc = dsi_panel_tx_cmd_set(display->panel, DSI_CMD_OSC_CLK_MODEO0);
             } else {
                 rc = dsi_panel_tx_cmd_set(display->panel, DSI_CMD_OSC_CLK_MODEO1);
             }
             if (rc)
                 pr_err("Failed to configure osc dynamic clk when wake-up!\n");
